...
function _asyncMap( eachfn, arr, iterator, callback ) {
	...
	if( ! callback ) {
		eachfn.call( this, arr2, function ( x, callback ) {
			iterator.call( this, x.value, function onResult( err ) {
				callback( err );
			});
		});
	}
...
Async.prototype.auto = function auto( tasks, callback ) {
	...
	function taskComplete() {
        ...
        for( let i=0, len=keys.length; i<len; i++ ) {
        	  ...
        	  , task      = Array.isArray(tasks[task_name]) ? tasks[task_name] : [tasks[task_name]];
...
Async.prototype.retry = function retry( times, task, callback ) {
	...
	function wrappedTask( wrappedCallback, wrappedResults ) {
		...
		Async.prototype.series.call( that, attempts,
			function onDone( done, data ) {
				data = data[ data.length - 1 ];
				(wrappedCallback || callback).call( that, data.err, data.result );
			});
	}
...
Async.prototype.waterfall = function waterfall( tasks, callback ) {
	callback = callback || noop;
	let context = this.ctx;
	if( tasks.constructor!==Array && Object.prototype.toString.call(tasks)!=='[object Array]' ) {
...
Async.prototype.priorityQueue = function priorityQueue( worker, concurrency ) {
	...
	var q = Async.prototype.queue.call( this, worker, concurrency );
	
	// Override push to accept second parameter representing priority
	q.push = function push( data, priority, callback ) {
